...
export default function CoinbaseWalletModal() {
  const { isOpen, selectedNode, modalType, closeModal } = useModalStore();
  const updateNode = useWorkflowStore((state) => state.updateNode);

  const [config, setConfig] = useState<CoinbaseWalletConfig>({
    inputText: "",
    fromToken: "0x833589fCD6eDb6E08f4c7C32D4f71b54bdA02913",
  });

  if (!isOpen || !selectedNode || modalType !== "coinbaseWallet") return null;
    ...
    <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50">
      <div className="bg-[#2A2A2A] text-white rounded-lg shadow-xl w-full max-w-md">
        <div className="flex justify-between items-center p-4 border-b border-gray-600">
          <h2 className="text-lg font-semibold">Coinbase wallet added</h2>
          <button
            onClick={closeModal}
          ...
          <div>
            <div>
              <div style={{ fontSize: 18, textAlign: "center" }}>
                You are now accessing the coinbase wallet
              </div>
            </div>
          </div>
