...
export function usePlaceholderClick() {
  ...
  const onFloatNodeClick = (id: NodeProps['id'], item: NavItem) => {
    ...
    const createFloatingNode = (
      ...
      type: NodeType.FLOAT,
      data: { label, parentId, isCollapsed: true },
    });
    ...
    if (camelCase(item.name) === NodeType.CONDITION) {
      ...
      const childFloatNode = {
        ...
        position: {
          x: parentNode.position.x + 100, // Adjust X position for centering
          y: parentNode.position.y, // Adjust Y position for centering
        },
        type: NodeType.FLOAT,
        data: { label: '+', parentId: parentNode.id, isCollapsed: false },
      };
        ...
        .map(node => {
          ...
          if (node.id === id) {
            return {
              ...
              data: { label: item.name, isCollapsed: true },
            };
          }
          return node;
        })
